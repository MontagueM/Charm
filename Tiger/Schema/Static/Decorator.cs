...
public class Decorator : Tag<SDecorator>
{
    ...
    public void LoadIntoExporter(ExporterScene scene, string saveDirectory)
    {
        ...
        List<Vector4> SpeedtreePlacements = new();
        ...
        for (int i = 0; i < _tag.InstanceRanges.Count - 1; i++)
        {
            ...
            for (int j = 0; j < count; j++)
            {
                ...
                {
                    Position = (SpeedtreePlacements[0] * pos + SpeedtreePlacements[1]).ToVec3(),
                    Quaternion = (SpeedtreePlacements[2] * rot + SpeedtreePlacements[3]),
                    Rotation = Vector4.QuaternionToEulerAngles((SpeedtreePlacements[2] * rot + SpeedtreePlacements[3])),
