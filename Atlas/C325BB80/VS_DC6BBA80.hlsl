// ---- Created with 3Dmigoto v1.3.16 on Wed Feb 15 23:57:53 2023
...
// unknown dcl_: dcl_input_sgv v4.x, instance_id
  ...
  // r2.xyzw becomes vertex world position without rotation
  r2.xyz = v0.xyz * cb1[0].www + cb1[0].xyz;
  r2.w = 1;
  
  SV calc vvv
  // cb1 is the world SRT matrix, or ignoring scale and translation is just rotation matrix
  // r1.w = object_position.z - camera_position.x = forward vector
  // r1.xyz = rotation_matrix_rowZ
  // r1.x = dot(rotation_matrix_rowZ, vertex world position)
  ...
  // r1.xyz should now be the vertex world position with rotation
  
  //o4.xyz = cb12[7].xyz + r1.xyz;
  
  // mul( output.Position, View )
  // r0.xyzw = view_matrix_rowX * vertex_world_position.x + view_matrix_rowY * vertex_world_position.y + view_matrix_rowZ * vertex_world_position.z
  // safe to say r0.xyzw = mul(view_matrix, vertex_world_position) ?
  ...
  r0.x = min(asuint(cb1[1].w), (uint)v3.x);
